    ...
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <Surface style={styles.levelCard} elevation={2}>
          ...
          <ProgressBar progress={(profile?.xp || 0) / 1000} color={COLORS.primary} style={styles.progress} />
        </Surface>
      </View>
      ...
      <ScrollView contentContainerStyle={styles.content}>
        ...
        <View style={styles.badgeGrid}>
          {badges.map(badge => (
            <Surface key={badge.id} style={[styles.badgeCard, !badge.unlocked && styles.lockedBadge]} elevation={1}>
              <IconButton 
                icon={badge.icon} 
                size={32} 
        ...
        <Surface style={styles.shopCard} elevation={2}>
          ...
          <Text variant="bodySmall" style={styles.shopSubtitle}>Unlock premium themes and soundscapes with XP</Text>
...
const styles = StyleSheet.create({
  ...
  badgeCard: { width: '47%', aspectRatio: 1, justifyContent: 'center', alignItems: 'center', borderRadius: ROUNDNESS.md, backgroundColor: COLORS.surfaceLow },
  lockedBadge: { opacity: 0.4 },
  badgeTitle: { color: COLORS.onSurface, marginTop: 4 },
  shopCard: { padding: SPACING.lg, borderRadius: ROUNDNESS.lg, backgroundColor: COLORS.surfaceHigh, borderStyle: 'dashed', borderWidth: 1, borderColor: COLORS.outlineVariant },
